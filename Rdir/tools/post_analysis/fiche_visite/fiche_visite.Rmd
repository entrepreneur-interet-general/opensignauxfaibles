...
```
**APE niveau 3**: 261  Fabrication de composants et cartes électroniques
**APE niveau 5**: `r first(etablissement$code_ape)` `r first(etablissement$libelle_naf_niveau5)`  
...
etablissement_with_past <- etablissement %>%
  ...
  mutate(activite_partielle_approx = effectif * ratio_apart / 100) %>%
  gather(c('activite_partielle_approx'),
  ...
  g1 <- ggplot(to_plot,
  aes(x = periode)) +
  geom_area(aes(y = nombre_heures, fill = conso_ou_demande)) +
