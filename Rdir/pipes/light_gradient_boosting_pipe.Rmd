...
raw_data <- connect_to_database('Features', last_batch, date_inf = date_inf, date_sup = date_sup, algo = algorithm, min_effectif = 30, fields = fields)

connect_to_database('Features', last_batch, algo = algorithm, min_effectif = 30, fields = c('siret','periode'))
...
raw_data$numero_compte_urssaf <- as.factor(paste(raw_data$numero_compte_urssaf))
all_data$numero_compte_urssaf <- as.factor(paste(all_data$numero_compte_urssaf))
...
prediction <- prediction %>%
  arrange(desc(prob)) %>% 
  filter(libelle_naf_niveau1 == "Industrie manufacturière", 
         connu == 0, 
