...
## Script parameters
```{r}
actual_period <- as.Date("2018-10-01")
last_batch = '1811'
algorithm = 'algo2'
min_effectif = 10
```
...
```{r}
res <- light_gradient_boosting(actual_period, last_batch, algorithm, min_effectif)
...
```{r results="hide"}
export_fiche_visite(sirets = c("95000399600044", "33362117500053", "34323685700045", "40256514700020", "35342749500018", "87695038700020"), last_batch, with_urssaf = FALSE)
```
...
pred_data %>% 
  ...
  prepare_for_export(export_fields = export_fields[-2], last_batch = last_batch, algorithm = algorithm) %>%
  export(batch = '1810')
```
